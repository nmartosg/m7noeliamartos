...
// AFEGEIX EL CODI ENVIAT VIA EMAIL A L'USUARI.
function afegirCodiRecuperar($codi, $email){
  $conn = connectDB('localhost', 'nmartos', 'nmartos', 'nmartos_a5');
  $data=date('Y-m-d H:i:s');
  $sql="INSERT INTO recoveryPassword (codi, email, data) VALUES ('$codi','$email','$data')";
  if (!$resultado =$conn->query($sql)){
      die("Erro:".$conn->error);
  }
  $conn->close();
...
////// FUNCIONS DEL CANVI DE CONTRASENYA


//COMPROVAR EMAIL DEL USUARI AL CANVIAR LA CONTRASENYA.
function comprovarEmailRecuperarPassword($codi){
  $conn = connectDB('localhost', 'nmartos', 'nmartos', 'nmartos_a5');
  $existe=false;
  $sql="SELECT recoveryPassword.email FROM recoveryPassword INNER JOIN usuaris ON usuaris.email=recoveryPassword.email WHERE recoveryPassword.codi='$codi'";
  if (!$resultado =$conn->query($sql)){
      die("Error:".$conn->error);
  }
  if($resultado->num_rows==1){
...
//COMPROVAR EL CODI SI EXISTEIX A L'EMAIL.
function EmailExitsCodi($codi){
  $conn = connectDB('localhost', 'nmartos', 'nmartos', 'nmartos_a5');
  $sql="SELECT email FROM recoveryPassword WHERE codi='$codi'";
  if (!$resultado =$conn->query($sql)){
      die("Error: ".$conn->error);
  }
...
function GetDataLink($codi){
  $conn = connectDB('localhost', 'nmartos', 'nmartos', 'nmartos_a5');
  $sql="SELECT data FROM recoveryPassword WHERE codi='$codi'";
  if (!$resultado =$conn->query($sql)){
      die("ErroR: ".$conn->error);
  }
  if($resultado->num_rows>=0){
...
//EDITAR LA CONTRASENYA DELL LINK EMAIL
function editarPassword($email, $password){
  $conn = connectDB('localhost', 'nmartos', 'nmartos', 'nmartos_a5');
  $sql="UPDATE usuaris SET password=md5('$password') WHERE email='$email'";
  if (!$resultado =$conn->query($sql)){
      die("Error: ".$conn->error);
