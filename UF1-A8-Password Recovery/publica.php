...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            $error=TRUE;
            $errormsg.= "<p>EMAIL INCORRECTE </p>";
        }
        $password = test_input($_POST["password"]);
        if (!preg_match("/^[a-zA-Z0-9' ]*$/",$password)) {
            $error=TRUE;
            $errormsg.=  "NOMÉS S'ACCEPTEN LLETRES O NUMEROS.";
        }
        if(!$error){
...
?>
    <!-- LOGIN USUARI -->
    <center><hr /><h2 style="color: #0000FF; ">LOGIN USUARI</h2><hr />
        <h1><?=$errormsg?></h1>
        <form method="post">
            <p>
                <label> EMAIL: </label>
        ...
        <br>
        <a href="#" onclick="check();"> Si has oblidat la contrasenya i la vols recuperar via email, clica a regenerar password</a><br></br>
